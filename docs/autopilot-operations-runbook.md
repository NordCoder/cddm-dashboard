...
4. Inspect the active Wave, circuit breakers, ambiguous records and stale-Head warnings.
5. Enable Autopilot with the current operator revision.
6. Observe the ordered Intent queue, lane leases, provisioning records, immutable command IDs and merge read-back records.
...
Before recovery, breaker resolution, delivery retry decisions or cleanup, record one correlated row from the current Autopilot projection. Do not combine values from different refreshes or infer a missing value.
...
| Scheduler | Intent ID, action ID, role, lane key, lease ID and claim ID |
| Provisioning | Provisioning request ID, managed worker/session ID, exact tab ID, observed ChatGPT URL, binding ID and binding version |
| Command delivery | Materialization ID, Workflow Command ID, delivery command ID, context hash and prompt hash |
| Result or merge read-back | Correlated result-comment identity or merge-cycle ID, approved Head and observed merge commit |

The lease token is a transition credential and must not be copied into operator notes. Record the lease ID and claim ID instead. If any required identity is absent, incompatible or belongs to another refresh, treat the projection as malformed, keep work paused and investigate rather than substituting a default.
...
1. Create the authoritative evidence table row before changing control state. Record Project, Wave, GitHub target, scheduler, provisioning, binding, command and result/merge identities as they appear.
...
4. Resume and verify one Intent claim, one lease, one provisioning request and one managed exact-tab worker. Update the same evidence row; do not start a second row for the same attempt.
5. Before bootstrap, restart the extension and verify the same provisioning request, worker, tab and binding are reused.
...
8. Deliver one assignment and verify its materialization ID, Workflow Command ID, delivery command ID, expected Head, context hash and prompt hash.
9. Restart during assignment delivery and verify the same ledger identity reports uncertain rather than sending again.
10. Publish one valid correlated result comment, synchronize GitHub and verify the expected next route against the original Intent and command identities.
...
14. For a merge attempt, record merge-cycle ID, approved Head and observed merge commit; reject any read-back that does not match the command-bound Issue and PR.
...
Stop and require owner review when GitHub facts conflict, the exact ChatGPT Project cannot be proven, attachment evidence drifts, a browser send is uncertain, result comments conflict, the projection omits an authoritative collection or identity, or merge read-back does not match the approved Head. Never infer success from ChatGPT response content.
