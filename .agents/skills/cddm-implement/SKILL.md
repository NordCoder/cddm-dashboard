...
6. Before returning `DONE`, run practical V2 checks from `AGENTS.md` for every affected surface and inspect the full diff.
7. Do not weaken tests, invariants, or policy to obtain green checks. Do not add secrets or local environment data.
8. Do not stage, commit, push, or write GitHub state; the host launcher creates/publishes the Candidate after a successful result.
9. Stop when Candidate-ready local state is satisfied. Do not perform unrelated cleanup.
