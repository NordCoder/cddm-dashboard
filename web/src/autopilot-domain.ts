...
export type AutopilotControl = { project_id: number; revision: number; last_action: string; updated_at: string }
export type AutopilotProfile = {
  project_id: number; resource_version: string; methodology_version: string; result_protocol: string
  delivery_mode: string; qa_session_mode: string; auto_merge: boolean; autonomy_mode: string; autonomy_state: string
  control_issue_number: number; max_active_work_units: number; max_parallel_implementors: number; max_parallel_qa: number
  chatgpt_project_url?: string; updated_at: string
}
export type AutopilotCounts = {
  pending_intents: number; blocked_intents: number; claimed_intents: number; active_leases: number
  pending_provisioning: number; managed_sessions: number; active_commands: number
  active_circuit_breakers: number; ambiguous_records: number
}
export type AutopilotIntent = {
  intent_id: string; project_id: number; source_result_comment_id: number; source_command_id: string; action_id: string
  action_type: string; repository: string; issue_number?: number; role?: string; pr_number?: number
  expected_head?: string; expected_previous_head?: string; reason_code?: string; decision_category?: string; wave_id?: string
  priority: number; lane_key?: string; status: string; created_at: string; updated_at: string
}
export type AutopilotQueueItem = { intent: AutopilotIntent; waiting_reason?: string }
export type AutopilotLease = {
  lease_id: string; project_id: number; lane_key: string; intent_id: string; claim_id: string; lease_owner: string
  status: string; acquired_at: string; expires_at: string; released_at?: string
}
export type AutopilotBreaker = {
  id: string; project_id: number; scope_kind: string; lane_key?: string; code: string; reason: string
  recovery_requirements: string; evidence?: string; expected_head?: string; status: string; occurrence_count: number
  created_at: string; updated_at: string; acknowledged_at?: string; resolved_at?: string
}
export type AutopilotProvisioning = {
  id: string; project_id: number; intent_id: string; lease_id: string; lane_key: string; issue_number: number; role: string
  expected_head?: string; status: string; completion_reason?: string; worker_id?: string; worker_session_id?: string
  tab_id?: number; observed_chatgpt_url?: string; bound_binding_id?: string; bound_binding_version?: number
  created_at: string; updated_at: string
}
export type AutopilotCommand = {
  project_id: number; materialization_id: string; intent_id: string; lease_id: string; provision_request_id: string
  lane_key: string; issue_number: number; role: string; expected_head?: string; status: string; reason_code?: string
  workflow_command_id?: string; workflow_status?: string; delivery_command_id?: string; delivery_status?: string
  worker_id?: string; worker_session_id?: string; tab_id?: number; binding_id?: string; binding_version?: number
  observed_chatgpt_url?: string; context_hash?: string; prompt_hash?: string; updated_at: string
}
export type AutopilotResult = {
  project_id: number; github_comment_id: number; issue_number: number; command_id: string; role: string; result: string
  payload_hash: string; validation_status: string; validation_reason?: string; accepted_at?: string; observed_at: string
}
export type AutopilotWarning = {
  code: string; intent_id?: string; issue_number?: number; pr_number?: number; expected_head?: string; observed_head?: string; message: string
}
export type AutopilotWave = {
  project_id: number; wave_id: string; control_issue_number: number; source_command_id: string; status: string
  issues: number[]; created_at: string; updated_at: string
}
export type AutopilotMergeCycle = {
  id: string; project_id: number; intent_id: string; issue_number: number; pr_number: number; approved_head: string
  observed_merge_commit?: string; status: string; reason_code?: string; updated_at: string
}
export type AutopilotStatus = {
  project_id: number; repository: string; sync_status: string; sync_error?: string; profile: AutopilotProfile
  control: AutopilotControl; active_wave?: AutopilotWave; intents: AutopilotIntent[]; queue: AutopilotQueueItem[]
  leases: AutopilotLease[]; active_leases: AutopilotLease[]; provisioning: AutopilotProvisioning[]
  commands: AutopilotCommand[]; results: AutopilotResult[]; circuit_breakers: AutopilotBreaker[]
  warnings: AutopilotWarning[]; merge_cycles: AutopilotMergeCycle[]; counts: AutopilotCounts
  project_hold_reason?: string; lead_busy: boolean; next_action: string; generated_at: string
}
...
function same(actual: unknown, expected: unknown, path: string, description: string): void {
  if (actual !== expected) throw new ValidationError(path, description)
}

function parseControl(value: unknown, path: string): AutopilotControl {
  const item = record(value, path)
  return { project_id: positiveInteger(item.project_id, `${path}.project_id`), revision: nonNegativeInteger(item.revision, `${path}.revision`), last_action: nonEmptyString(item.last_action, `${path}.last_action`), updated_at: nonEmptyString(item.updated_at, `${path}.updated_at`) }
}
function parseProfile(value: unknown, path: string): AutopilotProfile {
  const item = record(value, path)
  return {
    project_id: positiveInteger(item.project_id, `${path}.project_id`), resource_version: nonEmptyString(item.resource_version, `${path}.resource_version`),
    methodology_version: nonEmptyString(item.methodology_version, `${path}.methodology_version`), result_protocol: nonEmptyString(item.result_protocol, `${path}.result_protocol`),
    delivery_mode: nonEmptyString(item.delivery_mode, `${path}.delivery_mode`), qa_session_mode: nonEmptyString(item.qa_session_mode, `${path}.qa_session_mode`),
    auto_merge: booleanValue(item.auto_merge, `${path}.auto_merge`), autonomy_mode: nonEmptyString(item.autonomy_mode, `${path}.autonomy_mode`),
    autonomy_state: nonEmptyString(item.autonomy_state, `${path}.autonomy_state`), control_issue_number: nonNegativeInteger(item.control_issue_number, `${path}.control_issue_number`),
    max_active_work_units: positiveInteger(item.max_active_work_units, `${path}.max_active_work_units`), max_parallel_implementors: positiveInteger(item.max_parallel_implementors, `${path}.max_parallel_implementors`),
    max_parallel_qa: positiveInteger(item.max_parallel_qa, `${path}.max_parallel_qa`), chatgpt_project_url: optionalString(item.chatgpt_project_url, `${path}.chatgpt_project_url`),
...
function parseCounts(value: unknown, path: string): AutopilotCounts {
  const item = record(value, path)
  return {
    pending_intents: nonNegativeInteger(item.pending_intents, `${path}.pending_intents`), blocked_intents: nonNegativeInteger(item.blocked_intents, `${path}.blocked_intents`),
    claimed_intents: nonNegativeInteger(item.claimed_intents, `${path}.claimed_intents`), active_leases: nonNegativeInteger(item.active_leases, `${path}.active_leases`),
    pending_provisioning: nonNegativeInteger(item.pending_provisioning, `${path}.pending_provisioning`), managed_sessions: nonNegativeInteger(item.managed_sessions, `${path}.managed_sessions`),
    active_commands: nonNegativeInteger(item.active_commands, `${path}.active_commands`), active_circuit_breakers: nonNegativeInteger(item.active_circuit_breakers, `${path}.active_circuit_breakers`),
...
function parseIntent(value: unknown, path: string): AutopilotIntent {
  ...
  const parsed: AutopilotIntent = {
    intent_id: nonEmptyString(item.intent_id, `${path}.intent_id`), project_id: positiveInteger(item.project_id, `${path}.project_id`),
    source_result_comment_id: nonNegativeInteger(item.source_result_comment_id, `${path}.source_result_comment_id`), source_command_id: stringValue(item.source_command_id, `${path}.source_command_id`),
    action_id: nonEmptyString(item.action_id, `${path}.action_id`), action_type: nonEmptyString(item.action_type, `${path}.action_type`), repository: nonEmptyString(item.repository, `${path}.repository`),
    issue_number: optionalPositiveInteger(item.issue_number, `${path}.issue_number`), role: optionalString(item.role, `${path}.role`), pr_number: optionalPositiveInteger(item.pr_number, `${path}.pr_number`),
    expected_head: optionalSha(item.expected_head, `${path}.expected_head`), expected_previous_head: optionalSha(item.expected_previous_head, `${path}.expected_previous_head`),
    reason_code: optionalString(item.reason_code, `${path}.reason_code`), decision_category: optionalString(item.decision_category, `${path}.decision_category`), wave_id: optionalString(item.wave_id, `${path}.wave_id`),
    priority: integerValue(item.priority, `${path}.priority`), lane_key: optionalString(item.lane_key, `${path}.lane_key`), status: nonEmptyString(item.status, `${path}.status`),
    created_at: nonEmptyString(item.created_at, `${path}.created_at`), updated_at: nonEmptyString(item.updated_at, `${path}.updated_at`),
  }
  if (['dispatch', 'correct', 'merge_candidate'].includes(parsed.action_type)) {
  ...
  if ((parsed.action_type === 'dispatch' || parsed.action_type === 'correct') && !parsed.role) throw new ValidationError(`${path}.role`, 'worker role for dispatch or correction')
  if (parsed.pr_number !== undefined && !parsed.expected_head) throw new ValidationError(`${path}.expected_head`, 'exact Candidate Head when PR identity is present')
...
function parseLease(value: unknown, path: string): AutopilotLease {
  const item = record(value, path)
  return {
    lease_id: nonEmptyString(item.lease_id, `${path}.lease_id`), project_id: positiveInteger(item.project_id, `${path}.project_id`), lane_key: nonEmptyString(item.lane_key, `${path}.lane_key`),
    intent_id: nonEmptyString(item.intent_id, `${path}.intent_id`), claim_id: nonEmptyString(item.claim_id, `${path}.claim_id`), lease_owner: nonEmptyString(item.lease_owner, `${path}.lease_owner`),
    status: nonEmptyString(item.status, `${path}.status`), acquired_at: nonEmptyString(item.acquired_at, `${path}.acquired_at`), expires_at: nonEmptyString(item.expires_at, `${path}.expires_at`),
...
function parseBreaker(value: unknown, path: string): AutopilotBreaker {
  ...
  const parsed: AutopilotBreaker = {
    id: nonEmptyString(item.id, `${path}.id`), project_id: positiveInteger(item.project_id, `${path}.project_id`), scope_kind: nonEmptyString(item.scope_kind, `${path}.scope_kind`),
    lane_key: optionalString(item.lane_key, `${path}.lane_key`), code: nonEmptyString(item.code, `${path}.code`), reason: nonEmptyString(item.reason, `${path}.reason`),
    ...
    expected_head: optionalSha(item.expected_head, `${path}.expected_head`), status: nonEmptyString(item.status, `${path}.status`), occurrence_count: positiveInteger(item.occurrence_count, `${path}.occurrence_count`),
    created_at: nonEmptyString(item.created_at, `${path}.created_at`), updated_at: nonEmptyString(item.updated_at, `${path}.updated_at`), acknowledged_at: optionalString(item.acknowledged_at, `${path}.acknowledged_at`),
...
function parseProvisioning(value: unknown, path: string): AutopilotProvisioning {
  ...
  const parsed: AutopilotProvisioning = {
    id: nonEmptyString(item.id, `${path}.id`), project_id: positiveInteger(item.project_id, `${path}.project_id`), intent_id: nonEmptyString(item.intent_id, `${path}.intent_id`),
    lease_id: nonEmptyString(item.lease_id, `${path}.lease_id`), lane_key: nonEmptyString(item.lane_key, `${path}.lane_key`), issue_number: positiveInteger(item.issue_number, `${path}.issue_number`),
    role: nonEmptyString(item.role, `${path}.role`), expected_head: optionalSha(item.expected_head, `${path}.expected_head`), status: nonEmptyString(item.status, `${path}.status`),
    completion_reason: optionalString(item.completion_reason, `${path}.completion_reason`), worker_id: optionalString(item.worker_id, `${path}.worker_id`), worker_session_id: optionalString(item.worker_session_id, `${path}.worker_session_id`),
  ...
  if (parsed.status === 'provisioned') {
    ...
    if (!parsed.worker_session_id) throw new ValidationError(`${path}.worker_session_id`, 'managed session identity for provisioned request')
...
function parseCommand(value: unknown, path: string): AutopilotCommand {
  ...
  const parsed: AutopilotCommand = {
    ...
    intent_id: nonEmptyString(item.intent_id, `${path}.intent_id`), lease_id: nonEmptyString(item.lease_id, `${path}.lease_id`), provision_request_id: nonEmptyString(item.provision_request_id, `${path}.provision_request_id`),
    lane_key: nonEmptyString(item.lane_key, `${path}.lane_key`), issue_number: positiveInteger(item.issue_number, `${path}.issue_number`), role: nonEmptyString(item.role, `${path}.role`),
    expected_head: optionalSha(item.expected_head, `${path}.expected_head`), status: nonEmptyString(item.status, `${path}.status`), reason_code: optionalString(item.reason_code, `${path}.reason_code`),
    workflow_command_id: optionalString(item.workflow_command_id, `${path}.workflow_command_id`), workflow_status: optionalString(item.workflow_status, `${path}.workflow_status`),
    delivery_command_id: optionalString(item.delivery_command_id, `${path}.delivery_command_id`), delivery_status: optionalString(item.delivery_status, `${path}.delivery_status`),
    worker_id: optionalString(item.worker_id, `${path}.worker_id`), worker_session_id: optionalString(item.worker_session_id, `${path}.worker_session_id`), tab_id: optionalPositiveInteger(item.tab_id, `${path}.tab_id`),
...
function parseResult(value: unknown, path: string): AutopilotResult {
  ...
  return {
    ...
    issue_number: positiveInteger(item.issue_number, `${path}.issue_number`), command_id: nonEmptyString(item.command_id, `${path}.command_id`), role: nonEmptyString(item.role, `${path}.role`),
    result: nonEmptyString(item.result, `${path}.result`), payload_hash: nonEmptyString(item.payload_hash, `${path}.payload_hash`), validation_status: nonEmptyString(item.validation_status, `${path}.validation_status`),
    validation_reason: optionalString(item.validation_reason, `${path}.validation_reason`), accepted_at: optionalString(item.accepted_at, `${path}.accepted_at`), observed_at: nonEmptyString(item.observed_at, `${path}.observed_at`),
  }
}
function parseWarning(value: unknown, path: string): AutopilotWarning {
  ...
  return {
    ...
    expected_head: optionalSha(item.expected_head, `${path}.expected_head`), observed_head: optionalSha(item.observed_head, `${path}.observed_head`), message: nonEmptyString(item.message, `${path}.message`),
  }
}
function parseWave(value: unknown, path: string): AutopilotWave {
  const item = record(value, path)
  return {
    project_id: positiveInteger(item.project_id, `${path}.project_id`), wave_id: nonEmptyString(item.wave_id, `${path}.wave_id`), control_issue_number: positiveInteger(item.control_issue_number, `${path}.control_issue_number`),
    source_command_id: nonEmptyString(item.source_command_id, `${path}.source_command_id`), status: nonEmptyString(item.status, `${path}.status`),
    issues: requiredArray(item.issues, `${path}.issues`, (entry, itemPath) => positiveInteger(entry, itemPath)), created_at: nonEmptyString(item.created_at, `${path}.created_at`), updated_at: nonEmptyString(item.updated_at, `${path}.updated_at`),
  }
}
function parseMergeCycle(value: unknown, path: string): AutopilotMergeCycle {
  ...
  return {
    id: nonEmptyString(item.id, `${path}.id`), project_id: positiveInteger(item.project_id, `${path}.project_id`), intent_id: nonEmptyString(item.intent_id, `${path}.intent_id`),
    issue_number: positiveInteger(item.issue_number, `${path}.issue_number`), pr_number: positiveInteger(item.pr_number, `${path}.pr_number`), approved_head: shaValue(item.approved_head, `${path}.approved_head`),
    observed_merge_commit: optionalSha(item.observed_merge_commit, `${path}.observed_merge_commit`), status: nonEmptyString(item.status, `${path}.status`), reason_code: optionalString(item.reason_code, `${path}.reason_code`),
    updated_at: nonEmptyString(item.updated_at, `${path}.updated_at`),
  }
}
...
function compareLease(actual: AutopilotLease, expected: AutopilotLease, path: string): void {
  same(actual.project_id, expected.project_id, `${path}.project_id`, 'project identity matching durable lease')
  same(actual.intent_id, expected.intent_id, `${path}.intent_id`, 'Intent identity matching durable lease')
  same(actual.lane_key, expected.lane_key, `${path}.lane_key`, 'lane identity matching durable lease')
  same(actual.claim_id, expected.claim_id, `${path}.claim_id`, 'claim identity matching durable lease')
  same(actual.lease_owner, expected.lease_owner, `${path}.lease_owner`, 'lease owner matching durable lease')
  same(actual.acquired_at, expected.acquired_at, `${path}.acquired_at`, 'acquisition time matching durable lease')
  same(actual.expires_at, expected.expires_at, `${path}.expires_at`, 'expiry time matching durable lease')
...
function validateIdentityGraph(status: AutopilotStatus): void {
  ...
  if (status.profile.autonomy_mode === 'continuous_dashboard_orchestration' && status.profile.control_issue_number <= 0) throw new ValidationError('$.profile.control_issue_number', 'positive Control Issue for continuous Autopilot')
  ...
  status.intents.forEach((intent, index) => {
  ...
  status.leases.forEach((lease, index) => {
  ...
  status.commands.forEach((command, index) => {
    ...
    same(command.worker_session_id, request.worker_session_id, `${path}.worker_session_id`, 'managed session matching provisioning')
...
export function parseAutopilotStatus(value: unknown): AutopilotStatus {
  ...
  const parsed: AutopilotStatus = {
    ...
    lead_busy: booleanValue(item.lead_busy, '$.lead_busy'), next_action: nonEmptyString(item.next_action, '$.next_action'), generated_at: nonEmptyString(item.generated_at, '$.generated_at'),
  }
