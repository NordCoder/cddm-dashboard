...
export function WorkerLoopPanel(props: {
  ...
  return h(
    ...
    freshQARequired ? h('section', { className: 'callout callout--warning' }, h('strong', null, 'Fresh QA chat required'), h('p', null, props.chatCreationMode === 'automatic' ? 'Dashboard will create and bind a new QA conversation while this Work Unit remains open.' : 'Create or bind a new QA conversation. The previous QA binding is retired after an accepted terminal QA result.')) : null,
    ...
    h('section', { className: 'panel-section' },
      ...
      !props.chatCreationAvailable ? h('p', { className: 'muted' }, 'Reload the updated CDDM extension to enable fresh-chat creation.') : h('p', { className: 'muted' }, 'Automatic mode is stored for this Project in the current browser. Lead chat creation remains explicit.'),
    ),
